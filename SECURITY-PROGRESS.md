...
**המשתמש שנחסם — ההכרעה (Itzik, 7.8.2026):**

> תשובת השרת אחידה לכל המקרים, **בלי יוצא מן הכלל**. הטקסט המורחב מוצג לפי
> **מונה מקומי בדפדפן**, לא לפי תשובת השרת. הלקוח ממילא יודע כמה פעמים הוא
> עצמו ניסה, ולכן זה לא מדליף — בעוד שהצגה מותנית בתשובת השרת הופכת את
> העזרה עצמה לאורקל. בלי טיימר, בלי מספרים.

כלומר המבנה:

```ts
// השרת: תשובה אחת, תמיד. הקומפוננטה לא מסתכלת עליה כדי להחליט מה להציג.
const [localFails, setLocalFails] = useState(0);   // בזיכרון הקומפוננטה בלבד
...
```
CODE_INVALID  — תמיד, מהניסיון הראשון:
  "הקוד לא תקף.
   ייתכן שהוא שגוי, שכבר נעשה בו שימוש, או שפג תוקפו.
   בקש/י מבן/בת הזוג לייצר קוד חדש מהמסך שלהם."

תוספת מקומית — מוצגת מ-localFails >= 3, בלי קשר לתשובת השרת:
  "נראה שיש קושי. אפשר לנסות שוב מאוחר יותר, או לפנות אלינו."
  [כפתור: "דברו איתנו" → /contact]
```

**מה שנשלל במפורש:** טיימר ספירה לאחור, "נסה שוב בעוד N שניות", מספר ניסיונות
שנותרו, סטטוס 429, או כל הודעה שנבדלת בין "נחסמת" ל"קוד שגוי". שעון גלוי
מספר לתוקף מתי החלון מתאפס — וזה בדיוק האורקל שסגרנו, בדלת האחורית.

**למה המונה המקומי בטוח:** הוא נספר בזיכרון הקומפוננטה, מתאפס ברענון, ואינו
משפיע על שום החלטה בשרת. הוא לא **מגבלה** אלא **תצוגה** — האכיפה כולה נשארת
...
# שתי הבדיקות שביקשת — 7.8.2026

## 1. `DEFAULT` לא חל ב-`UPDATE` — בדקתי את `rotate_pair_code`

**ההיגיון שלך נכון, והמלכודת אמיתית.** במקרה הזה היא לא נפלה:

```sql
UPDATE public.couples
   SET pair_code            = v_new_code,
       pair_code_used_at    = NULL,
       pair_code_expires_at = now() + INTERVAL '14 days',   -- ← מפורש
       pair_code_rotated_at = now()
 WHERE id = p_couple_id;
```

הפונקציה **קובעת את התפוגה במפורש** ואינה נשענת על ברירת המחדל. אילו הייתה
נשענת עליה, הקוד המסובב היה יורש את התפוגה הישנה — ולזוג בן 20 יום היה נולד
כשהוא כבר פג.

**הוספתי טסט רגרסיה** בדיוק לתרחיש שתיארת, כדי שאיש לא "יפשט" את ההשמה
המפורשת בעתיד: זוג עם `created_at` לפני 20 יום ו-`pair_code_expires_at` שפג
לפני 6 ימים → סיבוב → `days_left = 14`, **וגם** הקוד החדש נפדה בפועל
(מה שלא היה קורה אילו ירש את החלון שפג).

```
✓ ד ... sets a FRESH expiry on rotation — a DEFAULT would not apply to an UPDATE
Tests  17 passed (17)
```

## 2. חריג הריפוי — התשובה המפורשת: אין מסלול קוד חי שעוקף

בדקתי את **כל** מסלולי יצירת הזוג:

**במיגרציות — שני `INSERT INTO public.couples` בלבד, שניהם דרך המחולל:**

| מקום | הפונקציה | שורה קודמת |
|---|---|---|
| `029:168` | `create_couple_for_self` | `v_pair_code := public.generate_pair_code();` (166) |
| `042:108` | `ensure_couple_for_user` | `v_pair_code := public.generate_pair_code();` (106) |

**בקוד האפליקציה — אפס.** כל 17 האזכורים של `.from("couples")` הם `select`,
פרט לשני `update`: `is_active: false` ב-`leaveCouple`, ו-`started_journey_at`
ב-`auto-assign`. **אף אחד מהם לא נוגע ב-`pair_code`.**

**המסקנה:** העקיפה היחידה האפשרית היא `INSERT` ידני בעורך ה-SQL. זה בדיוק
המקרה שאמרת שמקובל, וריפוי ב-backfill הוא התשובה הנכונה עבורו — **לא ממצא.**

לו היה מסלול קוד חי שעוקף, ריפוי לא היה פתרון בכלל, כי אין "הרצה הבאה" של
מיגרציה. תיעדתי את ההבחנה הזו בטסט עצמו כדי שהיא לא תיקבר.

---

# רשימת אימות אחרי-הרצה

`supabase/migrations/203_pair_code_hardening.postrun.sql` — להרצה **מיד אחרי**
ההחלה. הנחת היסוד שלך נכונה: **"המיגרציה רצה בהצלחה" אומר רק שלא הייתה שגיאת
תחביר.** שניים מארבעת הבאגים ב-203 היו בלתי נראים גם לביקורת וגם להחלה
מוצלחת.

חמש הבדיקות, כל אחת מוכיחה **תוצאה** ולא ביצוע:

| # | מוכיח | ציפייה | מה כישלון אומר |
|---|---|---|---|
| 1 | `column_default` על `pair_code_expires_at` | מכיל `now() + '14 days'` | ברירת מחדל ריקה = התפוגה מתה לכל זוג עתידי, וה-backfill מסתיר את זה |
| 2 | זוג חדש דרך `ensure_couple_for_user` | `days_left = 14`, `used_at` NULL | המסלול שה-Cardcom callback משתמש בו לא מקבל תפוגה |
| 3 | הקוד של אותו זוג שמור ב-`pair_codes_issued` | `is_reserved = true` | המחולל לא תופס → קוד חי עלול להינתן לזוג שני |
| 4 | סיבוב על זוג קיים | קוד שונה, `days_left = 14`, הישן עדיין שמור | ירושת תפוגה, או קוד שחוזר למאגר |
| 5 | הרשאות + RLS על שלוש הטבלאות | אפס שורות; שני הבוליאנים true | רשימת הקודים קריאה — התיקון מתהפך |

בדיקות 2 ו-3 עטופות ב-`BEGIN … ROLLBACK` ולכן אינן משאירות זוג בדיקה.
