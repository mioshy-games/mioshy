...
- **סטטוס:** ✅ **סגור באוויר** (9.8.2026) — קוד, טסטים, סוד מוגדר, ואומת בפרודקשן
...
## עובדות שנאספו בשטח (9.8.2026)

- **`WEBHOOK_ID = 2100356`** — ה-webhook היחיד שמצביע ל-`/api/brevo/unsubscribe-webhook`.
- **אין כפילות.** נבדק מול שלושת סוגי ה-webhooks; אין שני webhooks לאותו נתיב,
  ולכן אין סיכון ש-webhook שני ימשיך להישלח בלי כותרת ויידחה אחרי המיזוג.
- **אין inbound webhooks כלל.**
- אורך הסוד שנבחר: **43 תווים** → `token_len` התקין בפקודה 3 הוא **43**.
...
# H1 — סגור באוויר (9.8.2026)

**‏#52 מוזג ונפרס. `BREVO_WEBHOOK_SECRET` מוגדר בשני הצדדים והאכיפה פעילה.**

## איך הועבר הסוד

**ככותרת `Authorization: Bearer`, דרך ה-API של Brevo** — לא כפרמטר בכתובת.
webhook `2100356`, היחיד שמצביע לנתיב; אין כפילות ואין inbound webhooks.

## מה אומת בפועל, לפני המיזוג

הבדיקה בוצעה בבקשה מדומה (`curl`) ולא בהסרה אמיתית, כדי לא להוציא כתובת
מרשימת התפוצה. מול הקוד שהיה פרוס אז — שעדיין הבחין בין המקרים בתשובה:

| מה נשלח | מה חזר | מה זה הוכיח |
|---|---|---|
| הסוד הנכון בכותרת | `200` · `{"ok":true,"matched":false}` | הסוד **התקבל**, הבקשה עברה את בדיקת ההרשאה והגיעה ללוגיקה |
| סוד שגוי | `401` · `{"error":"unauthorized"}` | **האכיפה פעילה** — לא נכשלת-פתוח |

שתי התוצאות יחד הן ההוכחה: הערך תואם בין Brevo לוורסל, **וגם** שערך שגוי
נדחה. הבדיקה עם הכתובת הלא-קיימת לא כתבה דבר לבסיס הנתונים.

## מה השתנה מרגע המיזוג

הקוד החדש **מאחד את התשובות בכוונה**: כל תוצאה מאומתת מחזירה
`{"ok":true}` זהה, **וגם סוד שגוי מחזיר `200`** — כדי שלא ניתן יהיה להשתמש
בנתיב כדי לבדוק אם כתובת רשומה אצלנו.

**ההשלכה התפעולית:** מהיום, **קוד תגובה אינו ראיה**. הראיות היחידות הן:
1. שאילתת ההסרות (`marketing_consent_source = 'brevo_unsubscribe'`).
2. שורות `event=rejected scope=brevo.webhook` ביומן Vercel — כל דחייה
   נרשמת ברמת error ונספרת ב-error rate.

## מה עדיין פתוח סביב H1

- **מיגרציה 201 טרם הורצה** — היא אינה תלויה בקוד שנפרס ואינה נדרשת לו.
- **‏F22** — להסיר את קבלת `?secret=` אחרי מחזור דיוור מלא שבו הכותרת עבדה.
- **ביום הדיוור הראשון** — להריץ את שאילתת ההסרות. זו הפעם הראשונה שהנתיב
  פוגש נפח אמיתי.
