/**
 * Yaoshi – Spin Wheel Games Seed Runner
 * Uses Supabase service-role key to bypass RLS and insert all 7 games.
 */
...
// ─── helpers ─────────────────────────────────────────────────────────────────
function makeSlices(categories) {
  const slices = [];
  for (let i = 0; i < 2; i++) {
...
function makePlayerConfig(categories) {
  return {
    desired_total_slices: 10,
    categories: categories.map(c => ({
      id: `cat-${c.key}`, key: c.key,
      label_he: c.label_he, label_en: c.label_en, color: c.color,
    })),
...
const MARKER_CONFIG = {
  marker_type: 'none', marker_color: '#ffffff',
  marker_size: 8, marker_count: 0, marker_position: 0,
};
...
// ══════════════════════════════════════════════════════════════════
{
  ...
  slug: 'first-date-spin', bg_type: 'color', bg_value: '#1a0a2e',
  categories: [
  ...
  questions: [
    ...
    {t:'wouldyou',   l:'flirty',he:"לשמוע 'אני אוהב/ת אותך' פעם אחת מכל הלב, או לשמוע את זה בכל יום בקצרה?", en:"Hear 'I love you' once with full heart, or hear it briefly every day?"},
...
// ══════════════════════════════════════════════════════════════════
{
  ...
  slug: 'couple-heart-spin', bg_type: 'color', bg_value: '#1a0010',
  categories: [
    { key:'appreciation', label_he:'💕 הוקרה',      label_en:'💕 Appreciation',    color:'#E91E63' },
    { key:'deep_connect', label_he:'🔍 עומק',        label_en:'🔍 Deep Connection', color:'#9C27B0' },
  ...
  questions: [
    {t:'appreciation',l:'light', he:'אמור לבן/בת זוגך דבר אחד שהוא/היא עושה שגורם לך להרגיש בטוח/ה',     en:'Tell your partner one thing they do that makes you feel safe'},
    {t:'appreciation',l:'light', he:'מה הדבר שהכי אהבת בשבוע האחרון שהם עשו?',                              en:"What's one thing you loved most that they did in the past week?"},
    {t:'appreciation',l:'light', he:'ספר איך הם גרמו לך להרגיש אהוב/ה לאחרונה — בפרט אחד ספציפי',          en:'Tell how they made you feel loved recently — one specific detail'},
    ...
    {t:'deep_connect',l:'deep',  he:'מה הצורך הרגשי שלך שאתה/את מרגיש/ה שלא תמיד מתמלא?',                 en:"What emotional need do you feel isn't always met?"},
    ...
    {t:'memory',      l:'flirty',he:'מה היה הצחוק הכי גדול שלכם ביחד? חיקוי/ספר מחדש',                    en:'What was your biggest laugh together? Re-enact or retell it'},
    ...
    {t:'memory',      l:'flirty',he:"מה ה'בדיחה הפנימית' שלכם שרק שניכם מבינים?",                          en:"What's your inside joke that only the two of you understand?"},
    ...
    {t:'couple_dare', l:'flirty',he:'שבו פנים אל פנים, עין לעין 30 שניות — ואז בחרו: מה אתם רוצים לומר?', en:'Sit face to face, eye contact for 30 seconds — then choose: what do you want to say?'},
    ...
    {t:'future',      l:'deep',  he:'מה המנהג/מסורת שתרצו להקים ביחד כדי לחזק את הקשר?',                  en:"What custom/tradition would you want to establish together to strengthen your bond?"},
...
// ══════════════════════════════════════════════════════════════════
{
  ...
  slug: 'friends-party-spin', bg_type: 'color', bg_value: '#1a1200',
  categories: [
  ...
  questions: [
    ...
    {t:'truth_fun',   l:'light',he:'מה הדבר שאתה מתבייש להודות שאתה אוהב — סרט, אוכל, שיר?',              en:"What's something you're embarrassed to admit you love — movie, food, song?"},
    ...
    {t:'truth_fun',   l:'light',he:'אם מישהו אחר בחדר היה צריך לספר סיפור מביך עליך — מי זה?',             en:"If someone else in the room had to tell an embarrassing story about you — who?"},
    ...
    {t:'dare_silly',  l:'light',he:"כתוב הודעת וואטסאפ לאמא שלך ואמור לה 'אני כוכב לכת' — הראה לנו",     en:"Write a WhatsApp message to your mom saying 'I am a planet' — show us"},
    ...
    {t:'activity_now',l:'light',he:'מי שפתח אחרון את הטלפון — מציג את הסלפי האחרון שלו/ה לכולם',          en:'Whoever last opened their phone — shows their last selfie to everyone'},
    ...
    {t:'activity_now',l:'light',he:'כולם שרים ביחד 10 שניות של השיר הכי שנון שאפשר לחשוב עליו עכשיו',   en:'Everyone sings together 10 seconds of the catchiest song right now'},
    ...
    {t:'activity_now',l:'light',he:'כל אחד כותב שם חבר ומתאר אותו במילה אחת — ומקריא לכולם',             en:"Each writes a friend's name and describes them in one word — reads aloud"},
...
// ══════════════════════════════════════════════════════════════════
{
  ...
  slug: 'intimate-sparks-spin', bg_type: 'color', bg_value: '#0d0006',
  categories: [
  ...
  questions: [
    ...
    {t:'together_now',l:'flirty',he:"כל אחד בוחר שיר שמייצג אותו/ה — ושומעים ביחד בשתיקה",                 en:'Each picks a song that represents them — and you listen together in silence'},
    ...
    {t:'feel_share',  l:'deep',  he:"ספר מה מרגיש לך ה'בית' בקשר הזה — מה זה נראה?",                        en:"Tell what 'home' feels like in this relationship — what does it look like?"},
...
// ══════════════════════════════════════════════════════════════════
{
  ...
  slug: 'couple-renewal-spin', bg_type: 'color', bg_value: '#001020',
  categories: [
  ...
  questions: [
    ...
    {t:'unsaid_words',  l:'deep',  he:'מה ציפייה אחת שיש לך מהצד השני שמעולם לא ביטאת בבירור?',            en:"What's one expectation you have from your partner you've never clearly expressed?"},
    ...
    {t:'rediscover',   l:'flirty',he:'מה תשאל/י את הצד השני אם ידעת שהוא/היא ייענה/ייענו בכנות מלאה?',    en:"What would you ask your partner if you knew they'd answer with complete honesty?"},
    ...
    {t:'dare_reconnect',l:'flirty',he:'חבקו אחד את השני ו"תחזיקו" 90 שניות — כי אוקסיטוצין מתחיל שם',      en:'Hug each other and hold for 90 seconds — because oxytocin starts there'},
...
// ══════════════════════════════════════════════════════════════════
{
  ...
  slug: 'better-date-spin', bg_type: 'color', bg_value: '#001a0d',
  categories: [
  ...
  questions: [
    ...
    {t:'mini_adventure', l:'flirty',he:"קבעו 'בספונטני' יציאה לשבוע הבא — לאן שהאצבע תצביע במפה",          en:"Set a 'random' outing for next week — wherever your finger points on the map"},
    ...
    {t:'fun_challenge',  l:'light', he:"כל אחד מתחיל משפט ב'אנחנו תמיד...' — והשני מסיים אחרת",             en:"Each starts a sentence with 'We always...' — and the other finishes it differently"},
    ...
    {t:'plan_dream',     l:'deep',  he:'מה הפרויקט האישי שכל אחד מכם רוצה לממש השנה — ואיך תתמכו?',         en:"What's the personal project each of you wants to fulfill this year — and how will you support each other?"},
...
// ══════════════════════════════════════════════════════════════════
{
  ...
  slug: 'peak-desire-spin', bg_type: 'color', bg_value: '#0a0000',
  categories: [
  ...
  questions: [
    ...
    {t:'fantasy_bold', l:'deep',  he:'ספר/י על פנטזיית כוח — מי מוביל, מי נוהל — ואיך זה נראה?',            en:'Tell about a power fantasy — who leads, who follows — and what it looks like?'},
...
async function run() {
  ...
  for (const game of GAMES) {
    console.log(`\n▶ Processing: ${game.name_he} (${game.slug})`);
    ...
    const { data: gameRow, error: gErr } = await supabase
      ...
      .upsert({
        ...
        bg_type: game.bg_type, bg_value: game.bg_value,
        player_mode: false,
      }, { onConflict: 'slug' })
      .select('id')
      .single();

    if (gErr) { console.error('  ✗ game upsert:', gErr.message); continue; }
    const gameId = gameRow.id;
    console.log(`  ✓ game id: ${gameId}`);

    // 2. Upsert wheel config
    const slices       = makeSlices(game.categories);
    const playerConfig = makePlayerConfig(game.categories);
    const catColors    = makeCategoryColors(game.categories);
    ...
    if (wErr) { console.error('  ✗ wheel upsert:', wErr.message); continue; }
    console.log(`  ✓ wheel config saved (${slices.length} slices)`);

    // 3. Delete existing questions and re-insert
    await supabase.from('questions').delete().eq('game_id', gameId);

    const rows = game.questions.map(q => ({
      game_id: gameId, type: q.t, level: q.l,
      text_he: q.he, text_en: q.en, is_active: true,
    }));

    const { error: qErr } = await supabase.from('questions').insert(rows);
    if (qErr) { console.error('  ✗ questions insert:', qErr.message); continue; }

    console.log(`  ✓ ${rows.length} questions inserted`);
    totalInserted += rows.length;
  }

  console.log(`\n✅ Done! ${GAMES.length} games, ${totalInserted} questions total.`);
}
