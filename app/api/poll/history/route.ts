...
import { readPollAnonId, getPollUserId } from "@/lib/poll/anon";
import { getHistory, getHistoryWithTally } from "@/lib/poll/queries";

export async function GET(req: Request) {
  const anonId = await readPollAnonId();
  const userId = await getPollUserId(); // signed-in → history by user_id (cross-device)
  if (!anonId && !userId) return NextResponse.json({ history: [] });
  ...
  if (new URL(req.url).searchParams.get("tally") === "1") {
    return NextResponse.json({ history: await getHistoryWithTally(anonId, userId) });
  }
  return NextResponse.json({ history: await getHistory(anonId, userId) });
}
