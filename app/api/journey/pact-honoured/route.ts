...
const cronLog = makeLogger("cron.pact-honoured");
...
async function handle(req: Request): Promise<NextResponse<Summary>> {
  ...
  cronLog.info("run.ok", { processed: bumped, considered: (pacts ?? []).length, errors: errors.length, dur_ms: Date.now() - startedAt });
  return NextResponse.json({
    ok: errors.length === 0,
