...
export default async function JourneyMarketingPage({
  ...
  // Did the user finish the FREE short assessment? The short phase writes
  ...
  if (user) {
    ...
    const { data: jRow } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    const jStatus = (jRow as { status: string } | null)?.status ?? null;
    ...
    >
      ...
      <main className="relative">
        ...
        <div className="bg-white text-slate-900">
          ...
          {/* ════════════════════════════════════════════════════════════
              1. WEEKLY - the 5 domains, first cream section under the hero.
              Additive UI+copy only. Mobile-first; single centred column on
              desktop, matching the other cream sections.
          ════════════════════════════════════════════════════════════ */}
          {/* Weekly-program section — shared component (also used on the results
              page). CTA reuses primaryHref (active journey → timeline, else →
              assessment). */}
          <WeeklyProgramSection ctaHref={primaryHref} />
