...
### F19 — מאגר השאלות זז חמש פעמים בלי ששום שומר עצר
מ-6.5 עד 24.6.2026 מספר השאלות היה 32 → 29 → 26 → 30 → 28 → 29.
**אף שינוי מהם לא נעצר על ידי טסט**, למרות שקיים טסט שנועד בדיוק לזה
(`locked domain distribution`, `post love-language removal`).

הכשל האמיתי אינו שהשומר אדום — אלא **שאין שומר**: הטסט נכשל מאז השינוי
הראשון, ומאותו רגע כל ארבעת השינויים הבאים עברו בלי התנגדות. טסט אדום
שאינו חוסם merge שווה בדיוק כמו היעדר טסט, וכאן זה עלה במפרט שאיש לא
שם לב שנטש.

נגזרת ישירה של [F17]. הכרעת המוצר (האם המפרט תקף) ממתינה אצל Itzik —
לא לגעת בטסטים עד שתחזור.
...
### F22 — להסיר את קבלת `?secret=` מהנתיב של Brevo
`app/api/brevo/unsubscribe-webhook/route.ts` מקבל את הסוד **בשתי דרכים**:
ככותרת `Authorization: Bearer`, וכפרמטר `?secret=` בכתובת.

מ-9.8.2026 הפרודקשן משתמש **בכותרת בלבד** (ראה `SECURITY-PROGRESS.md`).
הפרמטר נשאר לעת עתה **כמסלול נסיגה** — אם הכותרת לא תיקלט, אפשר לחזור אליו
בשינוי כתובת אחד בלי פריסת קוד.

**מתי להסיר:** אחרי שהכותרת הוכיחה את עצמה בפרודקשן **לאורך מחזור דיוור אחד
מלא** — כלומר קמפיין גדול שבו נרשמו הסרות דרך שאילתה א', ואפס שורות
`event=rejected` ביומן Vercel.

**למה להסיר:** שטח קבלה שלא משתמשים בו לא צריך להישאר פתוח. כל עוד הפרמטר
מתקבל, סוד שידלוף ליומן גישה כלשהו (למשל אם מישהו יבנה כתובת עם `?secret=`
לצורך בדיקה) עדיין פותח את הנתיב. הסרה = שורתיים בקוד + עדכון הטסטים
שמכסים את המסלול הזה.

### F23 — תבנית הוואטסאפ `journey_reminder`: **לא לתקן**
`lib/whatsapp/templates.ts:40` — גוף התבנית: *"פרק חדש מחכה לכם…"*.

**זה נכון גם בלוגיקה החדשה.** התבנית לא טוענת שהקצב שבועי — היא מודיעה
שפרק ממתין, וזה מדויק גם כשחמישה פרקים נפתחים חודשית והזוג בוחר סדר.
**אין מה לתקן כאן.**

**הסיבה שזה נרשם:** מי שיעבור בעתיד על "כל המקומות שמדברים על פרקים" עלול
"לסדר" גם את זה. **העלות אינה אפס:** התבנית **מאושרת ב-Meta**, ושינוי נוסח
מחייב הגשה ואישור מחדש. עד שהאישור חוזר — השליחה נשברת.
(ההערה בקוד עצמו כבר אומרת שהשם וסדר המשתנים חייבים להתאים למה שאושר.)

**הכלל: לא נוגעים ב-`journey_reminder` בלי סיבה תוכנית אמיתית ובלי לתכנן
את חלון ההגשה מראש.**
