...
**לא לקומיט עדיין** (Itzik, 7.8.2026): קומיט בלי לאמת שתוכן הקובץ זהה למה
שרץ בפועל רק מקבע בגיט גרסה שאולי שגויה. קודם אימות מול `information_schema`,
אחר כך קומיט.

### F21 — אין רישום מיגרציות, וגיט אינו רישום אמין (שלב 4)
**המסקנה הרחבה מ-F20.** הרצות ידניות בעורך ה-SQL של Supabase אינן נרשמות
בשום מקום — אין טבלת ledger, אין כלי מיגרציות, אין קובץ. לכן **גיט הוא
הרישום היחיד, והוכח עכשיו שהוא לא אמין** (194 רצה ואינה בו).

תוצאה ישירה: **אי אפשר לענות על "האם מיגרציה N רצה".** הבדיקה שלי
ש-196–198 "פנויים" נשענה על `ls supabase/migrations/` — כלומר בדיוק על
המקור הלא-אמין. הניסוח נמשך.

**לשלב 4, שתי משימות:**
1. **התאמת סכמה מלאה** — להשוות את מצב ה-DB בפועל מול מה שרצף המיגרציות
   בגיט היה מייצר, ולתעד כל פער. זו הדרך היחידה לדעת מה עוד רץ ולא נרשם.

   **חייבת לכסות פונקציות, מדיניות RLS והרשאות — לא רק עמודות** (Itzik,
   7.8.2026). 194 הייתה עמודה ולכן בלטה; **פונקציה שתוקנה ידנית לא תיראה
   בשום השוואת סכמה שמסתכלת על טבלאות בלבד, וזה התרחיש הסביר יותר.**
   מקורות ההשוואה: `pg_get_functiondef` לכל פונקציה ב-`public`,
   `pg_policies`, `information_schema.role_table_grants`, ו-`pg_indexes` —
   לצד `information_schema.columns`.
2. **אימוץ רישום** — טבלת `schema_migrations` (או Supabase CLI) שמתעדת מה
   רץ ומתי, + כלל עבודה שאין הרצה ידנית בלי רישום.

**עד אז:** להמשיך מ-204 ומעלה ולא להניח דבר על 196–198.
